...
int lex_next(struct lexer *lx) {
    ...
    if (c == '<' && lx->src[lx->pos+1] == '=') { lx->pos += 2; return TOK_LE; }
    if (c == '>' && lx->src[lx->pos+1] == '=') { lx->pos += 2; return TOK_GE; }
    if (c == '&' && lx->src[lx->pos+1] == '&') { lx->pos += 2; return TOK_AND; }
    if (c == '|' && lx->src[lx->pos+1] == '|') { lx->pos += 2; return TOK_OR; }
